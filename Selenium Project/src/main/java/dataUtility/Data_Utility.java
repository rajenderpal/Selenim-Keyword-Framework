...
public class Data_Utility {
	...
	public Object[][] data_reader(String testcasename, Xls_Reader xlsx) {
		...
		while (!xlsx.getCellData(sheetname, 0, Testnamerow)
				.equalsIgnoreCase(testcasename)) {
			Testnamerow++;
		}
		System.out.println("Test Case Name Row in :- " + sheetname + " sheet:- " + Testnamerow);
		...
		while (!xlsx.getCellData(sheetname, 0, Testdatastartrow + testdatarows)
				.equals("")) {
			testdatarows++;
		}
		...
		int testdatacols = 0;
		while (!xlsx.getCellData(sheetname, testdatacols, Testdatacolnamerow)
				.equals("")) {
			testdatacols++;
		}
...
///////////////////// 3rd May ////////////////////////////////
	...
	public void TestCaseIsRun(String testcasename, Xls_Reader xlsx, ExtentTest ExtTest) {
		...
		if (!flag.equalsIgnoreCase("Y")){
			ExtTest.log(LogStatus.SKIP, "Test case is skipped as Runmode is set to 'N'");			
			throw new SkipException("Skipping the Testcase as Runmode is set to 'N'");
		}
	}

	public void TestCaseNameCheck(Xls_Reader xlsx, String testname, String sheetname) {
		int rows = xlsx.getRowCount(sheetname);
		int Testnamerow = 1;
		while (!xlsx.getCellData(sheetname, 0, Testnamerow).equalsIgnoreCase(testname)) {
			Testnamerow++;
			if (Testnamerow > rows) {
				throw new Error("Oops!..Testcase name " + testname + " does not exist in the " 
						+ sheetname + " sheet.");
			}
		}
	}
